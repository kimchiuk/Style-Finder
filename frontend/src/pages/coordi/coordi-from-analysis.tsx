...
const CoordiFromAnalysis = () => {
  const { parsing, clothId } = useParams<{ parsing: string; clothId: string }>();
  ...
  useEffect(() => {
    if (parsing == null || clothId == null) return;
    else if (parsing == 'CLOSET') getClosetCloth(clothId);
    else getHadoopCloth(clothId);
  }, [parsing, clothId]);

  // ClosetCloth item 조회
  const getClosetCloth = (clothId: string) => {
    // api 조회
    clothId;
    const data = { id: 2, imageUrl: '', image: '', categories: [], details: [], textures: [], part: '' };
  ...
  // HadoopCloset item 조회
  const getHadoopCloth = (clothId: string) => {
    // api 조회
    clothId;
    const data = { id: 2, imageUrl: '', image: '', categories: [], details: [], textures: [], part: '' };
...
export default CoordiFromAnalysis;
